...
from .constants import (
    ...
    FG, FONT_UI_B, BG_INPUT, BORDER, BORDER2, FG3, FG_YELLOW, FONT_UI, FONT_MONO,
    FONT_MONO_S, FONT_ICON, BG_ITEM, BG_HOVER, BG_SEL
)


class VSBtn(tk.Label):
    """Flat, crisp button. Filled surface + subtle border + hover tint."""

    def __init__(self, parent, text, cmd=None,
                 primary=False, danger=False, width=None, **kw):
        self._bg   = BG_BTN    if primary else (BG_DANGER  if danger  else BG_BTN_SEC)
        self._hbg  = BG_BTN_HOV if primary else (BG_DNG_HOV if danger  else BG_BTN_HOV2)
        self._bd   = BG_BTN    if primary else (BG_DANGER  if danger  else BORDER)
        self._hbd  = BG_BTN_HOV if primary else (BG_DNG_HOV if danger  else BORDER2)
        opts = dict(bg=self._bg, fg="#ffffff", font=FONT_UI_B,
                    padx=18, pady=6, cursor="hand2", relief="flat",
                    highlightthickness=1, highlightbackground=self._bd)
        if width: opts["width"] = width
        super().__init__(parent, text=text, **opts, **kw)
        self._cmd = cmd
        self.bind("<Enter>",    lambda _: self._tint(self._hbg, self._hbd))
        self.bind("<Leave>",    lambda _: self._tint(self._bg, self._bd))
        self.bind("<Button-1>", lambda _: self._cmd() if self._cmd else None)

    def _tint(self, bg, bd):
        try:
    ...
    def retag(self, text=None, bg=None, hbg=None):
        ...
        if bg:   self._bg = bg; self._tint(bg, self._bd)
...
class ClipRow(tk.Frame):
    def __init__(self, parent, rid, ts, content, on_select, on_dbl, **kw):
        ...
        self._all = [self, inner, meta, self._prev_lbl,
                 self._strip]
        for w in self._all:
    ...
    def set_alert(self, text: str = ""):
        ...
        try:
            if self._alert_text:
                self._alert_lbl.config(text=self._alert_text, fg=FG_YELLOW)
                self._alert_icon_lbl.config(text="⚠", fg=FG_YELLOW)
            else:
                self._alert_lbl.config(text="", fg=FG3)
                self._alert_icon_lbl.config(text="", fg=FG3)
        except Exception:
            pass
