...
from .constants import (BG_DIALOG, BORDER, BORDER2, BG_DANGER, FG3, FG, FG_GREEN,
                        FG_YELLOW, FG_RED, BG_TITLE, BG_ACCENT, FONT_UI_S, FONT_UI, FONT_UI_B)
...
class VSDialog(tk.Toplevel):
    def __init__(self, parent, title, width=420, height=180):
        ...
        tk.Frame(shell, bg=BG_ACCENT, height=2).pack(fill=tk.X)

        tbar = tk.Frame(shell, bg=BG_TITLE, height=32)
        tbar.pack(fill=tk.X)
        ...
        tk.Label(tbar, text="⬡", font=(FONT_UI[0], 9),
                 bg=BG_TITLE, fg=FG3).pack(side=tk.LEFT, padx=(10, 4), pady=6)
...
class ConfirmDialog(VSDialog):
    def __init__(self, parent, title, message,
        ...
        tk.Label(row, text=icon, font=(FONT_UI[0], 22),
...
class ExportDialog(VSDialog):
    def __init__(self, parent, count, filename, width=430, height=175):
        ...
        tk.Label(row, text="✔", font=(FONT_UI[0], 22),
