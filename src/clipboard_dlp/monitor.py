...
from .clipboard import paste as safe_paste
...
class Monitor(threading.Thread):
    def __init__(self, db, q: queue.Queue, interval=0.6, notifications=True, notify_cooldown=2.0):
        ...
        # Source capture is throttled: window lookups (xdotool / win32) are
        # comparatively expensive and should not run on every poll.
        self._src_cache = None
        self._src_cache_ts = 0.0
        self._src_cache_ttl = 3.0
        self._xdotool = shutil.which("xdotool") if os.name != 'nt' else None
    ...
    def _capture_source(self, text: str) -> str | None:
        """Best-effort source capture, throttled by a short cache.
        ...
        Only the window lookup (which spawns subprocesses / touches the GUI
        stack) is cached; the regex heuristics are cheap and run every time.
        """
        now = time.time()
        if self._src_cache is not None and (now - self._src_cache_ts) < self._src_cache_ttl:
            return self._src_cache
        result = self._window_source()
        if result is not None:
            self._src_cache = result
            self._src_cache_ts = time.time()
            return result
        return self._heuristic_source(text)

    def _window_source(self) -> str | None:
        ...
        # Try xdotool (fast, but not always installed — check once at startup)
        if self._xdotool:
            try:
    ...
    def _heuristic_source(self, text: str) -> str | None:
        try:
